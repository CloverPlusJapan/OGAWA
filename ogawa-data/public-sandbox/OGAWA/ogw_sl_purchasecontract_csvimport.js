...
function resultForm(csvfileid){  
	if(!isEmpty(csvfileid)){
		...
		if(!isEmpty(resultsCsv)){
			...
			var theLink = nlapiResolveURL('SUITELET', 'customscript_ogw_sl_pc_csvimport', 'customdeploy_ogw_sl_pc_csvimport');
			var backbtns = "window.location.href='"+devpUrl+theLink+"&whence='";
			form.addButton('backbtns', '戻る', backbtns);
			var subList = form.addSubList('list', 'list', 'CSVインポート結果一覧');
			subList.addField('csv_error', 'text', 'エラー');
			subList.addField('csv_sonum', 'text', '購入契約書番号 ');
			subList.addField('csv_entity', 'text', '仕入先 ');
			subList.addField('csv_eff', 'text', '次に基づく有効性 ');
			subList.addField('csv_curr', 'text', '通貨 ');
			subList.addField('csv_strat', 'text', '契約開始日 ');
			subList.addField('csv_end', 'text', '契約終了日');
			var cont = 1;
			for(var i = 1 ; i  < fileArr.length ; i++){
				if(!isEmpty(fileArr[i])){
					 ...
					 var csv_sonum=fileLine[1];// 購入契約書番号
					 var csv_entity=fileLine[2];// 仕入先
					 var csv_eff=fileLine[3];// 次に基づく有効性
					 var csv_curr=fileLine[4];// 通貨
					 var csv_strat=fileLine[5];// 契約開始日
					 var csv_end=fileLine[6];// 契約終了日
