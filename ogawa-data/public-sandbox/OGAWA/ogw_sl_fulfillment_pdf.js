...
define(['N/render', 'N/file', 'N/search', 'N/runtime', 'N/format', 'N/config', 'N/record', 'N/url', 'N/xml'], function (render, file, search, runtime, format, config, record, url, xml) {
    ...
    function onRequest(context) {
        ...
        // get detail data
        var detailDic = getDetailData(searchResults, columns,headerDic);
    ...
    function getPdfData(recordId) {
        ...
        var columns = [
            search.createColumn({name: "trandate", join: "createdFrom"}),
            ...
            search.createColumn({name: "inventorynumber", join: "inventoryDetail",sort: search.Sort.ASC, label: " 编号"}),
            ...
            search.createColumn({name: "shipaddress1", join: "customer", label: "addr1"}),
            search.createColumn({name: "shipaddress2", join: "customer", label: "addr2"}),
            search.createColumn({name: "companyname", join: "customer"}),
            search.createColumn({name: "subsidiary", join: "customer"}),
            // search.createColumn({name: "fax", join: "customer"}),
        ];
    ...
    function getHeaderData(searchResults, columns) {
        ...
        var createFrom = searchResults[0].getValue(columns[10]);
        if (!isEmpty(createFrom)) {
            var orderDate = searchResults[0].getValue(columns[0]);
            headData.orderDate = formatPdfDate1(orderDate);
            headData.createFrom =  searchResults[0].getText(columns[10]).split("#")[1];
        ...
        var comName = searchResults[0].getValue(columns[14]);
        headData.comName = comName;


        var comSubsidiary = searchResults[0].getText(columns[15]).split(":")[1]||"";
        if (comSubsidiary.indexOf('&') != -1){
            comSubsidiary = comSubsidiary.replace('&', '&amp;');
        }
        headData.comSubsidiary =comSubsidiary

        // var comFax = searchResults[0].getValue(columns[16]);
        // headData.comFax = comFax;
     ...
     * @returns
     */
    function getDetailData(searchResults, columns,headerDic) {
        ...
        for (var i = 0; i < searchResults.length; i++) {
            ...
            var detailDataObj = {};
            detailDataObj.no = i+1;

            detailDataObj.createdfrom = tempResult.getValue(columns[10]);
            ...
            detailDataObj.stockQty = toThousands(Number(tempResult.getValue(columns[8])));

            QtySum+=Number(tempResult.getValue(columns[8]));
        ...
        headerDic.QtySum = toThousands(QtySum);
