...
function userEventBeforeLoad(type, form, request){
	...
	if(type=='view'){ 
		...
		// changed by song add 23030322 start
		setFieldHidden(record);
		// changed by song add 23030322 end
...
//				var mBodyCc=mtemplateRecord.custrecord_ogw_cc; //CC
...
//				if(!isEmpty(mBodyCc)&& isEmpty(poRecord.getFieldValue('custbody_ogw_cc'))){
//					poRecord.setFieldValue('custbody_ogw_cc', mToAddress); //発注書のCC
//				}
	...
	try {
			...
			if (poCustomform == po_ogj_custForm) {
				...
				//change by song add 23030413  socsv start 
				var poPurchasecontract =  poRecord.getFieldValue('purchasecontract'); //購入契約書
				if(!isEmpty(poPurchasecontract)){
					poRecord.setFieldValue('custbody_ogw_purchasecontract_list', poPurchasecontract);//購入契約書list
				}
				//change by song add 23030413  socsv end
				...
				if(!isEmpty(createdfrom)){
					...
					if(soCustomform == so_ogj_custForm){
//						var numCopyFlg = poRecord.getFieldValue('custbody_ogw_so_numcopy'); //SO番号コピー
//						if(numCopyFlg == 'T' && !isEmpty(soTranid)){
//							poRecord.setFieldValue('tranid', soTranid);
//						}
						
						//change by song add 23030512   start
//						var purchasecontract = soRecord.getFieldValue('custbody_ogw_purchasecontract_select');//購入契約書
//						if(!isEmpty(purchasecontract)){
//							if(isEmpty(poRecord.getFieldValue('purchasecontract'))){
////								poRecord.setFieldValue('purchasecontract', purchasecontract);//購入契約書
//							}
//							if(isEmpty(poRecord.getFieldValue('custbody_ogw_purchasecontract_list'))){
//								poRecord.setFieldValue('custbody_ogw_purchasecontract_list', purchasecontract);//購入契約書list
//							}
//						}
//						var poCount=poRecord.getLineItemCount('item');
//						for(var p=1;p<poCount+1;p++){
//							poRecord.selectLineItem('item', p);
//							poRecord.setCurrentLineItemValue('item', 'purchasecontract', purchasecontract); 
//							poRecord.commitLineItem('item');	
//						}
						//change by song add 23030512   end
...
//		if(type == 'dropship'){
			...
			if(!isEmpty(createdfrom)){
				var soRecord = nlapiLoadRecord('salesorder', createdfrom); 
				var soCustomform = soRecord.getFieldValue('customform');//SOカスタム・フォーム
				if(soCustomform == so_ogs_custForm){
					var entity = soRecord.getFieldValue('entity');//顧客
					if(!isEmpty(entity)){
						var entityTerms = nlapiLookupField('customer', entity, 'terms');
						if(!isEmpty(entityTerms) && entityTerms == '17' || entityTerms == '35' || entityTerms == '16' || entityTerms == '59'){
