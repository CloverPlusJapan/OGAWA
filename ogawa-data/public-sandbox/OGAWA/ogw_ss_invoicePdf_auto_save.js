...
function scheduled(type) {
    try{
        ...
        var param = nlapiGetContext().getSetting('SCRIPT', 'custscript_owg_data_json');
        nlapiLogExecution('DEBUG', 'param', param);
        var paramJson = JSON.parse(param);
        // var folderId = paramJson.folderId;
        for (var z = 0; z <  paramJson.dataArr.length; z++) {
            try {
                var invoiceId = paramJson.dataArr[z];
                ...
                if (custForm == invoice_ogj_custform) { //OGJ
                    ...
                    var tranId = invoiceRecord.getFieldValue("tranid") //請求書番号
                    ...
                    if (itemList != 0) {
                        for (var s = 1; s <= itemList; s++) {
                            ...
                            if (s == 1) {
                                itemCode = invoiceRecord.getLineItemText('item', 'item', s);//アイテム
                                itemId = invoiceRecord.getLineItemValue('item', 'item', s);//アイテム
                            }
                            ...
                            var etaValue = defaultEmpty(invoiceRecord.getLineItemValue('item', 'custcol_eta', s));//ETA//0504
                    ...
                    str += '</tr>';
                    str += '<tr>';
                    str += '<td style="padding: 0px 0px 0px 40px;" >' + operatorName + '</td>';
                    str += '</tr>';
                    str += '<tr>';
...
//			str += 'font-family: NotoSans, NotoSansCJKjp, sans-serif;';
                    ...
                    str += '<th align="left" colspan="4" class="titlebox" >&nbsp;&nbsp;納入日</th>';//0504
                    ...
                    for (var j = 0; j < invoiceItem.length; j++) {
                        ...
                        str += '<td align="left" colspan="4" class="titleboxbot">&nbsp;&nbsp;' + invoiceItem[j].etaValue + '</td>'; //ETA0504
                    ...
                    if (itemCodeArr.length == 3) {
                        xlsFile.setName(itemCodeArr[2] + '_' + itemCodeArr[1] + "_" + tranId + '.pdf');
                    } else {
                        var inventoryitemSearch = nlapiSearchRecord("inventoryitem", null,
                            ...
                            [
                                ...
                                new nlobjSearchColumn("entityid", "preferredVendor", null)
                            ]
                        );
                        if (inventoryitemSearch != null && inventoryitemSearch.length > 0) {
                            vendorCode = inventoryitemSearch[0].getValue('entityid', 'preferredVendor');
                        }
                        // var itemFive = itemCode.substring(itemCode.length - 5, itemCode.length);
                        xlsFile.setName('_' + vendorCode + "_" + tranId + '.pdf');
                    }
                    xlsFile.setFolder(folderId);
                    xlsFile.setIsOnline(true);
                    var fileID = nlapiSubmitFile(xlsFile);
                    nlapiLogExecution('DEBUG', 'lookUsage', parseInt(nlapiGetContext().getRemainingUsage()));
                    governanceYield();
                }
            }catch (e) {
...
function governanceYield() {
     if (parseInt(nlapiGetContext().getRemainingUsage()) <= 200) {
         var state = nlapiYieldScript();
         if (state.status == 'FAILURE') {
             nlapiLogExecution('AUDIT', 'Failed to yield script.');
